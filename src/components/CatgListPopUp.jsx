...
function CatgListPopUp({ isCatgList, setIsCatgList }) {
  const handleClick = () => {
  ...
  return (
    <div className={ds.catglistpopup}>
      <div className={ds.catghd}>
        <p>Make it yours</p>
        <button onClick={handleClick}>X</button>
      </div>
      <p className={ds.catgsel}>
        Select topics and interests to customize your Discover experience
      </p>
      <div className={ds.catglistmenu}>
        {listitem.map((item, index) => (
          <div key={index} className={ds.menuitem}>
            {item.img}
            <p>{item.name}</p>
          </div>
        ))}
      ...
      <button className={ds.svint} onClick={handleClick}>
        Save Interests
      </button>
    </div>
  );
